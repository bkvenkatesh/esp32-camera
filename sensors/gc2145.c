...
static int reset(sensor_t *sensor)
{
    ...
    if (ret == 0) {
        ...
        // write_reg(sensor->slv_addr, 0x05, 0); // buf_CISCTL_capt_hb[12:8]
        // write_reg(sensor->slv_addr, 0x06, 200); // buf_CISCTL_capt_hb[7:0]
...
static const uint16_t (*paraset[])[2]={
    gc2145_96x96_55fps,
    gc2145_160x120_50fps,
    gc2145_176x144_45fps,
    gc2145_240x176_40fps,
    gc2145_240x240_35fps,
    gc2145_320x240_30fps,
    gc2145_400x296_22fps,
    gc2145_480x320_20fps,
    gc2145_640x480_17fps,
    gc2145_800x600_10fps,
    gc2145_1024x768_5fps,
    gc2145_1280x720_10fps,
    gc2145_1280x1024_4_5fps,
    gc2145_1600x1200_4fps,
};

// static const uint16_t _low_pclk[][2]={
// ///////sdgdjfghgnbdsfhdfgfdhgdrtfhdhtrdjfgndfgseryhghfgjfgbhth
//     {0xfe, 0x00},
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    // write_reg(sensor->slv_addr, 0xfe, 0x00);
    // write_reg(sensor->slv_addr, P0_ROW_START_HIGH, H8(row_s)); // Row_start[10:8]
    // write_reg(sensor->slv_addr, P0_ROW_START_LOW, L8(row_s)); // Row_start[7:0]
    // write_reg(sensor->slv_addr, P0_COL_START_HIGH, H8(col_s)); // Column_start[10:8]
    // write_reg(sensor->slv_addr, P0_COL_START_LOW, L8(col_s)); // Column_start[7:0]
    // write_reg(sensor->slv_addr, 0x91, 0x00);
    // write_reg(sensor->slv_addr, 0x92, 0x00);
    // write_reg(sensor->slv_addr, 0x93, 0x00);
    // write_reg(sensor->slv_addr, 0x94, 0x00);
    // write_reg(sensor->slv_addr, P0_WIN_HEIGHT_HIGH, H8(h + 8)); //window_height [10:8]
    // write_reg(sensor->slv_addr, P0_WIN_HEIGHT_LOW, L8(h + 8)); //window_height [7:0]
    // write_reg(sensor->slv_addr, P0_WIN_WIDTH_HIGH, H8(w + 8)); //window_width [10:8]
    // write_reg(sensor->slv_addr, P0_WIN_WIDTH_LOW, L8(w + 8)); //window_width [7:0]
    ...
    write_regs(sensor->slv_addr, paraset[framesize]);
